...
const storageAdd = () => {
  ...
  let storageData = JSON.parse(localStorage.getItem("myPatrolReservations"));
  ...
  // データを再度保存
  localStorage.setItem("myPatrolReservations", JSON.stringify(storageData));
};
...
<template>
  <dialog id="reserve-modal" class="modal">
    ...
    <form class="form">
      <p class="form__label">{{ props.selectedInfo.pointName }}</p>
      <p class="form__label">
